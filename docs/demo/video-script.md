...
finished film is delivered at 1.5x, which is about 2:53 - see Timing And Budget, and read the flag under it.**
...
| Field              | Detail                                                                                                                                                                                                                                                             |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Runtime target** | 4:15-4:45 in real time, hard bound. 758 words of narration is 259.9s (about 4:20). The cut film is expected a little longer once the recorder's natural gaps between lines and beats are in, and is then delivered at 1.5x, about 2:53                             |
...
| **Delivered runtime at 1.5x**                  | 260s / 1.5 = 173.3s, about 2:53. The precise total delivers as 173.3s too; the two figures round to the same second                                                                                 |
| **Organisers' outer bound**                    | 3:00-5:00, strictly, per `docs/brief.md`. The real-time ladder clears it on both sides. **The 1.5x delivered film does not clear the floor** - see the note below                                   |
...
regenerated. The two figures, stated together so neither can be quoted without the other: the ladder is **260s (4:20) in
real time**, and the delivered film is **260 / 1.5 = 173.3s, about 2:53**. The ladder itself stays in real time and is
not restated at 1.5x anywhere, because the recorder, `schedule.py` and every offset in the narration block all work in
real time.

**FLAG: 2:53 IS BELOW THE ORGANISERS' FLOOR. `docs/brief.md` records the rule as "Strictly 3 - 5 minutes," a floor as
well as a ceiling, and a 2:53 delivery misses it by seven seconds.** This script's real-time ladder was written to a
4:15-4:45 hard bound precisely so the film would sit mid-band; dividing it by 1.5 puts the delivered runtime outside the
band the submission is checked against. **THIS IS NOT RESOLVED HERE.** Three ways out, none of them chosen by this
document: deliver at a lower speed, where 1.25x lands 208s (3:28) and 1.15x lands 226s (3:46); or keep 1.5x and lengthen
the narration, where 788 words carries the ladder to 270s (4:30) and delivers exactly 3:00 with no margin at all, and
831 words - 73 more than this script has - carries it to 285s (4:45) and delivers 3:10, which is the only lengthening
with any margin in it and also pins the real-time film to the very top of its own hard bound; or confirm with the
organisers that the rule measures the uploaded file and accept a 2:53 upload knowingly. The first is the cheapest and
...
buttons stay disabled and the Handbook is unreachable. Pin each row by its title, not its position: tick five in listed
order, Passport through Travel insurance, and leave JR Pass alone, since it starts ticked and a click would untick it.
Then press The Handbook, hold about two seconds, and let `shot-10` press The Book in the rail, not `page.goto`.
...
| `shot-9`  | 8s            | 16s         | +8s   | Five ticks, both buttons unlocking together, and a two-second hold on the Handbook still                        |
...
shot-9 | 400 | One gate before The Book. Six items, pulled straight from the trip itself: a teamLab ticket, the JR Pass already ticked off, because this trip never leaves Tokyo. She ticks the rest, and both unlock: The Book, and the Handbook, the same calendar's packing list and advice.
