...
**At a glance: 754 words, 258.5s of narration at 175 words per minute (about 4:19), inside the 4:15-4:45 hard bound.**
...
| Field              | Detail                                                                                                                                                                                                                                                             |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Runtime target** | 4:15-4:45, hard bound. 754 words of narration is 258.5s (about 4:19). The cut film is expected a little longer once the recorder's natural gaps between lines and beats are in                                                                                     |
...
| `shot-1`-`shot-10` | The ten walkthrough beats                                                                           | budgeted | 204.0s      |
...
| `shot-7` (6 lines) | 2:21-3:05     | 44      | 128     |
| `shot-8`           | 3:05-3:15     | 10      | 29      |
| `shot-9`           | 3:15-3:31     | 16      | 47      |
| `shot-10`          | 3:31-3:52     | 21      | 60      |
| `stack`            | 3:52-4:06     | 14      | 41      |
| `close`            | 4:06-4:20     | 14      | 40      |
| **Total**          | **0:00-4:20** | **260** | **754** |

Each row's Seconds is that beat's own lines, individually rounded to the nearest second and summed - the same rule the
detailed tables below use, so a row here always matches its rows there. The precise, unrounded total is 258.5s (4:18.5);
summing already-rounded lines instead adds 1.5s, landing this ladder at 260s (4:20).
...
| **`shot-7` (i) - Apply - This Is The Moment** | 7       | 19    | `/desk`. Click Apply. Cards animate into their slots. All four day headers take a green colour chip.                                                                                                                                                           | "Then, Apply. One button, and the calendar colours itself, all four days green, every stop fits, door to door."                                                                                                                                                                                                                       |
...
**This Is The Moment is `shot-7`, all six lines of it.** Apply cannot produce gold by itself - it lays every day out in
its own optimal order, so the ratio the code checks is exactly 1.0 and every day comes up green. Gold only exists
...
| **Precise narration duration**                 | 754 words / 175 wpm = 258.5s, about 4:19                                                                                                                                                            |
...
| **Hard bound**                                 | 4:15-4:45 (255-285s). The precise total clears the floor by 3.5s and sits 26.5s under the ceiling; the ladder clears it by 5s and sits 25s under                                                    |
...
directly: `shot-7` today is `click Apply, wait for a placed card and a state chip, pause 1.5s` - nothing in it drags a
card back out. `v2/src/lib/schedule.ts` confirms why the beat had to change: `GOLD_SLACK` is 1.25 and gold is only
reachable when the day's actual transit exceeds the scheduler's own order for the same stops by more than 25 percent.
Apply lays every day out in that same order, so it cannot produce gold by construction - every day comes up green.
`shot-7`'s capture needs a second gesture added after Apply: drag one placed card into a worse position on one day (an
out-and-back, away from its cluster and back), then hold on that day's chip turning gold with its rationale line.
`shot-8`'s pin sequence is unaffected and unchanged.
...
| `shot-7`  | 13s           | 44s         | +31s  | Six lines cover Apply going all-green, the overrule drag, the gold flip, and its rationale text                 |
...
shot-7 | 400 | Then, Apply. One button, and the calendar colours itself, all four days green, every stop fits, door to door.
shot-7 | 7314 | Now watch her overrule it. She drags one stop out of order on purpose, away from its cluster and back again.
shot-7 | 14914 | That day's chip turns gold. Right beside it, in words: this order spends more minutes in transit than the scheduler's own.
shot-7 | 22514 | Not broken. Slow. The instrument doesn't stop her, it just answers back, the moment she overrules it.
shot-7 | 28743 | She can leave it like this, or drag that stop right back. Either way, it's her call, and the calendar just told her the cost of it.
shot-7 | 38400 | That's the whole idea. Not a clever schedule, an honest one, that tells her exactly what she's trading when she moves something herself.
shot-8 | 400 | One more move. She pins Sensoji so it can never drift, then hits Apply again, the rest of that day rebuilds around it, and Sensoji doesn't move an inch.
