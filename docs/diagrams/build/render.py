...
COLORS = {
 "1": ("#fde8e6", "#c0392b", "#7d1f16"),
 "2": ("#fdeee0", "#d1701a", "#8a4711"),
 "3": ("#fdf6dd", "#b08900", "#6d5500"),
 "4": ("#e6f6ea", "#2f8f4e", "#1c5c32"),
 "5": ("#e3f2f7", "#1e7f96", "#12505e"),
 "6": ("#efe9f7", "#6b46a8", "#432a6b"),
 None: ("#ffffff", "#c9c6bf", "#3a3733"),
}
...
def node_svg(n):
    fill, stroke, fg = COLORS.get(n.get("color"), COLORS[None])
    ...
    if n.get("file"):
        o.append(f'<text x="{x+PAD}" y="{y+34}" font-size="13" fill="{stroke}" '
                 f'font-family="ui-monospace,Consolas,monospace">&#128196; live note</text>')
        o.append(f'<text x="{x+PAD}" y="{y+62}" font-size="14.5" font-weight="600" fill="{fg}" '
                 f'font-family="ui-monospace,Consolas,monospace">{esc(os.path.basename(n["file"]))}</text>')
        o.append(f'<text x="{x+PAD}" y="{y+86}" font-size="12" fill="#8a867e" '
                 f'font-family="ui-monospace,Consolas,monospace">{esc(os.path.dirname(n["file"]))}/</text>')
        return "".join(o)
    ty = y + PAD
    for kind, rs, fs, lh in wrap(n["text"], w):
        ...
        for txt, st in rs:
            ...
            if "m" in st: a.append('font-family="ui-monospace,Consolas,monospace" font-size="%.1f"' % (fs*0.92))
...
def edge_svg(e, byid):
    ...
    p = (f'<path d="M{x1:.0f},{y1:.0f} C{c1[0]:.0f},{c1[1]:.0f} {c2[0]:.0f},{c2[1]:.0f} '
         f'{x2:.0f},{y2:.0f}" fill="none" stroke="#9b968d" stroke-width="1.9" '
         f'marker-end="url(#ar)"/>')
    if e.get("label"):
        ...
        p += (f'<rect x="{mx-wpx/2:.0f}" y="{my-12:.0f}" width="{wpx:.0f}" height="23" rx="11.5" '
              f'fill="#fffdf8" stroke="#c9c6bf" stroke-width="1.2"/>'
              f'<text x="{mx:.0f}" y="{my+4:.0f}" font-size="11.5" fill="#6a665e" '
...
def render(path):
    ...
    return (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="{x0:.0f} {y0:.0f} '
            ...
            f'<path d="M0,0 L10,5 L0,10 z" fill="#9b968d"/></marker></defs>'
            f'<rect x="{x0:.0f}" y="{y0:.0f}" width="{x1-x0:.0f}" height="{y1-y0:.0f}" fill="#faf8f4"/>'
            f'{body}</svg>'), (x1-x0, y1-y0)
