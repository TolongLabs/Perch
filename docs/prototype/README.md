...
| [`travel-planner-slides.html`](travel-planner-slides.html)  | 6 Sept     | A slide deck, six sections: the trip, plan-it-together with dates, slot voting, hotels, a day map and a photo matcher, photo spots, where to shop, cautions and news, and a read-only final plan carrying the Google Maps handoff |
...
| **Where to shop**       | A drawer per market or street with a weighted score from three review sources, what it is good for, what to watch for, and a verdict. The gift list is the last drawer |
