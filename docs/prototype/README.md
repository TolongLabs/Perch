...
| **Eat, shop, do**       | Three tabs, one accordion, **with pictures** - a thumbnail per drawer and three inside. A drawer per place with a weighted score from three review sources, plus a **RedNote post count kept outside the score** - when attention is high and the rating is not, it says so. Gift list is the last drawer under Shop |
