...
| **Notes on our picks**  | **Derived.** Only the places that won the vote, grouped by day: rough cost, how long, how to get there, practical notes, the photo line, and what catches people out |
| **Eat, shop, do**       | Three tabs, one accordion, **with pictures** - a thumbnail per drawer and three inside. Every place is **measured against the voted plan**: on it, a short detour from it, or a trip of its own, with a filter. Shop leads with what to bring home and the local-versus-strip price; eat and do lead with the place. A drawer per place with a weighted score from three review sources, plus a **RedNote post count kept outside the score** - when attention is high and the rating is not, it says so. Gift list is the last drawer under Shop |
